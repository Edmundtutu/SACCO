                    ...
                    <div id="memberOptionsOverlay" class="member-overlay-card position-absolute bg-white shadow-lg rounded-3 border d-none">
                        <div class="p-3 border-bottom bg-light rounded-top">
                            <h6 class="mb-0 text-center fw-bold text-dark">
                                <i class="bi bi-plus-circle me-2 text-primary"></i>Choose Membership Kind
                            </h6>
                        </div>

                        <div class="p-2">
                            <a href="{{ route('admin.members.create', ['type' => 'individual']) }}"
                               class="d-flex align-items-center text-decoration-none p-3 rounded-2 mb-2 member-option-link border">
                                <div class="me-3">
                                    <i class="bi bi-person-fill fs-4 text-primary"></i>
                                </div>
                                <div class="flex-grow-1">
                                    <div class="fw-semibold text-dark">Individual Member</div>
                                    <small class="text-muted">Register a single person as a member</small>
                                </div>
                            </a>

                            <a href="{{ route('admin.members.create', ['type' => 'vsla']) }}"
                               class="d-flex align-items-center text-decoration-none p-3 rounded-2 mb-2 member-option-link border">
                                <div class="me-3">
                                    <i class="bi bi-people-fill fs-4 text-success"></i>
                                </div>
                                <div class="flex-grow-1">
                                    <div class="fw-semibold text-dark">VSLA Group</div>
                                    <small class="text-muted">Register a small group like a Village Savings Association</small>
                                </div>
                            </a>

                            <a href="{{ route('admin.members.create', ['type' => 'mfi']) }}"
                               class="d-flex align-items-center text-decoration-none p-3 rounded-2 member-option-link border">
                                <div class="me-3">
                                    <i class="bi bi-building-fill fs-4 text-warning"></i>
                                </div>
                                <div class="flex-grow-1">
                                    <div class="fw-semibold text-dark">MFI Institution</div>
                                    <small class="text-muted">Register a Microfinance Institution like a Sacco</small>
                                </div>
                            </a>
                        </div>
                    </div>
    ...
    @push('scripts')
        <script>
            $(document).ready(function() {
                ...
                const memberOptionsOverlay = document.getElementById('memberOptionsOverlay');

                if (addMemberBtn && memberOptionsOverlay) {
                    // Show overlay on hover
                    addMemberBtn.addEventListener('mouseenter', function() {
                        memberOptionsOverlay.classList.remove('d-none');
                    });

                    // Hide overlay when mouse leaves both button and overlay
                    const hideOverlay = function() {
                        setTimeout(function() {
                            if (!addMemberBtn.matches(':hover') && !memberOptionsOverlay.matches(':hover')) {
                                memberOptionsOverlay.classList.add('d-none');
                            }
                        }, 100);
                    };

                    addMemberBtn.addEventListener('mouseleave', hideOverlay);
                    memberOptionsOverlay.addEventListener('mouseleave', hideOverlay);

                    // Close overlay when clicking outside
                    document.addEventListener('click', function(e) {
                        if (!addMemberBtn.contains(e.target) && !memberOptionsOverlay.contains(e.target)) {
                            memberOptionsOverlay.classList.add('d-none');
                        }
                    });
                }
