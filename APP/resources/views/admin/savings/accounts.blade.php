        ...
        <div class="row align-items-end">
            <div class="col-md-5 mb-3">
                ...
                <input type="text" name="search" class="form-control form-control-lg" 
                       placeholder="Search by member name, email, account number..." 
        ...
        <div class="mt-3">
            ...
            @if(request('search'))
                <span class="filter-badge">
                    ...
                    <a href="{{ route('admin.savings.accounts', array_merge(request()->except('search'))) }}" class="text-white ms-2">×</a>
                </span>
            @endif
            @if(request('status'))
                <span class="filter-badge">
                    Status: {{ ucfirst(request('status')) }}
                    <a href="{{ route('admin.savings.accounts', array_merge(request()->except('status'))) }}" class="text-white ms-2">×</a>
                </span>
            @endif
        </div>
        @endif
                ...
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <strong class="text-success">UGX {{ number_format($account->balance, 0) }}</strong>
                            </td>
                            <td>
                                <span class="badge bg-{{ $account->status == 'active' ? 'success' : ($account->status == 'inactive' ? 'danger' : 'warning') }}">
                                    {{ ucfirst($account->status) }}
...
$(document).ready(function() {
    ...
    $('#tableViewBtn').on('click', function() {
        $(this).addClass('active').removeClass('btn-outline-primary').addClass('btn-primary');
        $('#cardViewBtn').removeClass('active').removeClass('btn-primary').addClass('btn-outline-primary');
        $('#cardView').hide();
        $('#tableView').show();
    });
});
