            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <button type="button" class="btn btn-outline-warning btn-sm w-100 mb-2" data-bs-toggle="modal"
                                    ...
                                    @foreach ($stats['recent_transactions'] as $transaction)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <small
                                                    class="text-muted">{{ $transaction->account->user->member_number ?? 'N/A' }}</small>
                                            </td>
                                            ...
                                            <td>
                                                @switch($transaction->status)
                                                    ...
                                                    @default
                                                        <span
                                                            class="badge-status default">{{ ucfirst($transaction->status) }}</span>
                                                @endswitch
                                            </td>
                                            <td>
                                                <a href="{{ route('admin.savings.accounts.show', $transaction->account->id) }}"
                                                    class="btn btn-outline-primary btn-sm" title="View Account">
                                                    <i class="bi bi-eye"></i>
                                                </a>
                                            </td>
                                        </tr>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="payment_reference"
                                        name="payment_reference" placeholder="Optional payment reference">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                ...
                <div class="card-header">
                    <h5 class="mb-0">
                        ...
                        <button class="btn btn-sm btn-outline-primary float-end"
                            onclick="adminTransactionManager.loadTransactionData()">
        ...
        $(document).ready(function() {
            ...
            $('#member_search').on('input', function() {
                ...
                if (search.length >= 3) {
                    ...
                    $('#member_id').val('');
                    $('#account_id').prop('disabled', true).html(
                        '<option value="">Select account after choosing member</option>');
                }
            });
            ...
            async function searchMembers(query) {
                try {
                    ...
                    if (response.ok) {
                        ...
                        $('#member_results').html('<div class="alert alert-warning">No members found</div>')
                            .show();
                    }
                } catch (error) {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        const accountOptions = accounts.map(account =>
                            `<option value="${account.id}">${account.account_number} - ${account.accountable->name} (UGX ${parseFloat(account.balance).toLocaleString()})</option>`
                        ).join('');

                        $('#account_id').html('<option value="">Select account</option>' + accountOptions).prop(
                            'disabled', false);
                    } else {
                        $('#account_id').html('<option value="">No accounts found</option>').prop('disabled',
                            false);
                    }
                } catch (error) {
                    console.error('Error loading accounts:', error);
                    $('#account_id').html('<option value="">Error loading accounts</option>').prop('disabled',
                        false);
                }
            }
